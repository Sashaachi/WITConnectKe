...
public class forgotPassword extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        pswdEmail= findViewById(R.id.et_regEmail);
        resetPswd=findViewById(R.id.btn_reset);
        firebaseAuth=FirebaseAuth.getInstance();

        resetPswd.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                String usermail= pswdEmail.getText().toString().trim();
                if (usermail.equals("")){
                    ...
                    firebaseAuth.sendPasswordResetEmail(usermail).addOnCompleteListener(new OnCompleteListener<Void>() {
                        ...
                        public void onComplete(@NonNull Task<Void> task) {
                            if (task.isSuccessful()){
                                ...
                                Toast.makeText(forgotPassword.this,"Failed to send email check if email is correct",Toast.LENGTH_SHORT).show();
                            }
